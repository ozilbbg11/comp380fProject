...
public class LectureRepositoryImpl implements LectureRepository {
        ...
        public List<Lecture> extractData(ResultSet rs) throws SQLException, DataAccessException {
            ...
            while (rs.next()) {
                ...
                if (lecture == null) {
                    lecture = new Lecture();
                    lecture.setId(id);
                    lecture.setCustomerName(rs.getString("name"));
    ...
    @Override
    @Transactional
    public long createLecture(final String customerName, final String subject,
    ...
    public List<Lecture> getLectures() {
        final String SQL_SELECT_lectureS = "select l.*, a.filename, a.content_type,"
                + " a.content from lecture as l left join attachment as a"
                + " on l.id = a.lecture_id";
        return jdbcOp.query(SQL_SELECT_lectureS, new LectureExtractor());
    }

    @Override
    @Transactional(readOnly = true)
    public List<Lecture> getLecture(long id) {
        final String SQL_SELECT_lecture = "select l.*, a.filename, a.content_type,"
                + " a.content from lecture as l left join attachment as a"
                + " on l.id = a.lecture_id where l.id = ?";
